    ...
    >
      ...
      <div class="page info-page">
        ...
        <div class="info-group">
            ...
            <div class="info-actions">
              ...
              >
                <SvgIcon
                  ...
                  :size="26"
                  :class="{ liked: dataStore.isLikeSong(musicStore.playSong) }"
                />
              </div>
              <!-- 添加到歌单 -->
              <div
                class="action-btn"
                @click.stop="mobilePlayerSongMenuRef?.open(musicStore.playSong)"
              >
                <SvgIcon name="More" :size="26" />
              </div>
            </div>
          </div>
          <MobilePlayerSongMenu
            ref="mobilePlayerSongMenuRef"
            @click-quality="mobileSongQualityMenuRef?.open(musicStore.playSong)"
          />
          ...
          <div class="control-section">
            ...
            >
              <SvgIcon :size="24" :name="statusStore.playModeIcon" />
            </div>
      ...
      <div class="page lyric-page">
        <div class="lyric-header">
          <s-image :src="musicStore.getSongCover(-1)" class="lyric-cover" />
          ...
          <div
            class="action-btn"
            @click.stop="
              toLikeSong(musicStore.playSong, !dataStore.isLikeSong(musicStore.playSong))
            "
          >
            <SvgIcon
              :name="dataStore.isLikeSong(musicStore.playSong) ? 'Favorite' : 'FavoriteBorder'"
...
const mobilePlayerSongMenuRef = ref<InstanceType<typeof MobilePlayerSongMenu> | null>(null);
const mobileSongQualityMenuRef = ref<InstanceType<typeof MobileSongQualityMenu> | null>(null);
...
const { direction, isSwiping, lengthX } = useSwipe(mobileStart, {
  ...
  onSwipeEnd: () => {
    ...
    if (direction.value === "left" && lengthX.value > 100) {
      pageIndex.value = pageIndex.value === totalPage - 1 ? pageIndex.value : pageIndex.value + 1;
    } else if (direction.value === "right" && lengthX.value < -100) {
      pageIndex.value = pageIndex.value === 0 ? pageIndex.value : pageIndex.value - 1;
    }
    swipeOffset.value = 0;
  },
});
