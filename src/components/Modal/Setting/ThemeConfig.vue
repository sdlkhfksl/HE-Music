<template>
  ...
  >
    <div class="config-section">
      ...
      <n-card class="set-item">
        <div class="label">
          <n-text class="name">{{ t("setting.general.theme_variant") }}</n-text>
          <n-text class="tip" :depth="3">{{ t("setting.general.theme_variant_tip") }}</n-text>
        </div>
        <n-select
      ...
      <n-card class="set-item">
        <div class="label">
          <n-text class="name">{{ t("setting.general.custom_background") }}</n-text>
          <n-text class="tip" :depth="3">{{ t("setting.general.custom_background_tip") }}</n-text>
        </div>
        <div class="bg-actions">
          ...
          <n-button size="small" type="primary" strong secondary @click="selectBackgroundImage">
            {{ isCustomBackground ? t("common.change") : t("common.select") }}
          </n-button>
        </div>
        <input
          ref="fileInputRef"
    ...
    >
      <n-text class="section-title" :depth="2">{{ t("setting.general.theme_color") }}</n-text>
      <div class="color-grid">
        <div
          ...
          :title="t(`color.${snakeCase(colorData.label)}`)"
          @click="
        ...
        >
          ...
          <template v-if="key !== 'custom'">
            ...
            <n-text class="color-name" :depth="2">{{
              t(`color.${snakeCase(colorData.label)}`)
            }}</n-text>
          </template>
          <!-- 自定义颜色 -->
          <template v-else>
            ...
            <n-text class="color-name" :depth="2">{{ t(`color.${colorData.label}`) }}</n-text>
          </template>
        </div>
      </div>
    </div>
    ...
    <div v-else class="config-section">
      <n-card class="set-item">
        <div class="label">
          <n-text class="name">{{ t("common.scale") }}</n-text>
          <n-text class="tip" :depth="3">{{
            t("setting.general.custom_background_scale_tip")
          }}</n-text>
        </div>
        <n-slider
      ...
      <n-card class="set-item">
        <div class="label">
          <n-text class="name">{{ t("setting.general.custom_background_mask_color") }}</n-text>
        </div>
        <n-color-picker
          v-model:value="statusStore.backgroundConfig.maskColor"
          :show-alpha="true"
          :modes="['rgb']"
          placement="top"
          style="width: 200px"
      ...
      <n-card class="set-item">
        <div class="label">
          <n-text class="name">{{ t("setting.general.custom_background_blur") }}</n-text>
        </div>
        <n-slider
    ...
    <div v-if="isCustomBackground" class="color-section">
      ...
      <div class="color-grid">
        ...
        >
          ...
          <n-text class="color-name" :depth="2">{{ t("common.auto") }}</n-text>
        </div>
        <!-- 纯色模式 -->
        <div
          class="color-item"
        ...
        >
          ...
          <n-text class="color-name" :depth="2">{{ t("common.solid") }}</n-text>
        </div>
        <!-- 自定义颜色 -->
        <div
          class="color-item"
        ...
        >
          ...
          <n-text class="color-name" :depth="2">{{ t("color.custom") }}</n-text>
        </div>
      </div>
    </div>
  </n-scrollbar>
...
const variantOptions = [
  { label: t("setting.general.theme_variant_value_primary"), value: "primary" },
  { label: t("setting.general.theme_variant_value_secondary"), value: "secondary" },
  { label: t("setting.general.theme_variant_value_tertiary"), value: "tertiary" },
  { label: t("setting.general.theme_variant_value_neutral"), value: "neutral" },
  { label: t("setting.general.theme_variant_value_neutral_variant"), value: "neutralVariant" },
  { label: t("setting.general.theme_variant_value_error"), value: "error" },
];

// 主题颜色数据
const themeColors = themeColor as Record<string, { label: string; color: string }>;
