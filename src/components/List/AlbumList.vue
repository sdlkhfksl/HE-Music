    ...
    <div v-if="data.length > 0" class="cover-list album">
      <div class="cover-grid">
        <div
          ...
          class="cover-item"
          @click="goDetail(item)"
          @contextmenu="coverMenuRef?.openDropdown($event, item, 'album')"
        >
          <!-- 封面 -->
          <div class="cover">
            <s-image
...
// 是否处于当前播放列表
const isPlaying = (item: AlbumInfo | UserFavouriteAlbumInfo) =>
  musicStore.isPlayingPlaylist(item.id, item.platform, "album") && statusStore.playStatus;

// 查看详情
const goDetail = (item: AlbumInfo | UserFavouriteAlbumInfo) => {
  router.push({
    name: "album",
...
const playList = debounce(
  async (item: CoverType) => {
    try {
      // 是否为当前列表
      if (musicStore.isPlayingPlaylist(item.id, item.platform, "album"))
        return player.playOrPause();
      // 开始加载
      item.loading = true;
      // 获取播放列表
      const list = await albumDetail(item.id, item.platform);
